...
def jacobi(A, b, x0, tol, niter):
    ...
    x = x0.copy()
    iteraciones = []
    # Número de condición de la matriz (constante durante el método)
    ...
    for k in range(niter):
        ...
        # Errores
        e_abs_vec = np.abs(np.array(x_new) - np.array(x))
        error_abs = np.max(e_abs_vec)
        relative_err1 = np.max(np.abs(e_abs_vec / np.where(np.array(x_new)!=0, np.array(x_new), 1e-12)))
        relative_err2 = np.max(np.abs(e_abs_vec / np.where(np.array(x)!=0, np.array(x), 1e-12)))

        iteraciones.append([k+1, *x_new, error_abs, relative_err1, relative_err2, condicion])
        if error_abs < tol:
            break
        x = x_new
    return iteraciones
