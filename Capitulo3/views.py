...
def resultado_interpolacion(request):
    ...
    if metodo == "vandermonde":
        try:
            ...
            if y_real is not None and punto_eval is not None:
                y_ap = float(pol.evalf(subs={x_sym: punto_eval}))
                resultado["errores"] = _compute_all_errors(y_real, y_ap)
        ...
        try:
            ...
            if y_real is not None and punto_eval is not None:
                y_ap = next(
                    float(s.evalf(subs={x_sym: punto_eval}))
                    for s, (xi, xf) in spl
                    if xi <= punto_eval <= xf
                )
                resultado["errores"] = _compute_all_errors(y_real, y_ap)
        ...
        try:
            ...
            resultado["splines_cubicos"] = spl_cu
            if y_real is not None and punto_eval is not None:
                y_ap = float(spl_cu(punto_eval))
                resultado["errores"] = _compute_all_errors(y_real, y_ap)
        except ImportError:
